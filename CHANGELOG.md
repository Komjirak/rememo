...
### Added (Obsidian 자동 내보내기)
- **기간 필터**: 수동 내보내기 시 전체/최근 7일/30일/90일 중 선택 가능.
- **Vault 폴더 지정 + 백그라운드 자동 내보내기**: 설정에서 iOS 문서 피커로 Obsidian Vault 폴더를 한 번 선택(Security-Scoped Bookmark로 지속 접근 권한 확보)해두면, 이후 매일/매주/매월 주기로 새 메모를 그 폴더(`Rememo Export/`)에 자동으로 직접 쓴다. 사용자 개입이나 공유 시트 없이 동작.
- iOS `BGTaskScheduler`(BGAppRefreshTask) 기반 — 정확한 실행 시각은 보장되지 않으며 기기 사용 패턴에 따라 iOS가 결정한다(공식 제약). 네이티브에는 4시간 간격으로 "확인하러 오는" 것만 요청해두고, 실제 내보내기 여부는 Dart 쪽에서 "마지막 실행 + 사용자가 고른 주기"로 판단해 실제 빈도가 선택한 주기에 가깝게 유지되도록 설계.
- "지금 내보내기" 수동 테스트 버튼으로 폴더 쓰기 로직을 즉시 확인 가능.
- 신규 네이티브 파일 `ObsidianExportHelper.swift`(Security-Scoped Bookmark 관리/폴더 쓰기), `com.rememo.komjirak/obsidian_folder` MethodChannel, `workmanager` 패키지 연동.

### Added (Obsidian 내보내기)
- **설정 → 데이터 → "옵시디언으로 내보내기"** 추가. 모든 메모를 Obsidian 호환 마크다운(YAML 프론트매터 + 요약/핵심 포인트/개인 메모/원본 텍스트)으로 변환해 zip으로 묶은 뒤 OS 공유 시트로 전달한다. 서버/계정 없이 완전히 기기 내에서 처리 — 저장 위치로 iCloud Drive의 Vault 폴더를 선택하면 이후 PC와 자동 동기화된다(진짜 실시간 동기화는 아니며, 내보내기를 누를 때마다 해당 시점 스냅샷이 저장 위치에 반영되는 방식).
- 카드에 지정된 폴더가 있으면 `폴더명/제목.md`로, 없으면 `Inbox/제목.md`로 배치. 로컬 이미지는 `attachments/`에 함께 담아 `![[attachments/파일명]]`으로 임베드.
- 순수 문자열 변환 로직(마크다운 생성, 파일명 sanitize)에 유닛 테스트 10건 추가.
